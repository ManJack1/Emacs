...
*** 使用方法
- 调用 =my-toggle-deer= 切换 deer 模式

** 打开 PDF 文件的函数
*** 介绍
使用外部 Sioyek PDF 阅读器打开当前 buffer 中的 PDF 文件。
...
*** 使用方法
- 在 PDF buffer 中调用 =open-in-sioyek=
- 会在 Sioyek 阅读器中打开当前 PDF 文件


** 窗口调整函数
*** 介绍
使用 h/j/k/l 键连续调整窗口大小的交互式函数。
...
** PDF 渲染颜色
*** 介绍
将 PDFTools 的配色固定为 doom-one 主题风格，提供更好的阅读体验。
...
** Org 数学预览函数
*** 介绍
在保存和打开 Org 文件时自动预览数学公式，排除 config.org 以避免性能问题。
...
** 自动 Tangle 配置函数
*** 介绍
保存 config.org 时自动执行 org-babel-tangle 生成 config.el，提高配置管理效率。
...
** LSP UI 侧边栏切换函数
*** 介绍
切换 lsp-ui-imenu 侧边栏的显示和隐藏，方便查看代码结构。
...
** 智能 Tab 键函数
*** 介绍
智能 TAB 键实现，根据上下文自动选择行为：YASnippet 展开/跳转 > Copilot 补全 > Tabout > 普通缩进。
...
** dirvish
*** 介绍 
dirvish 是一个现代化的文件浏览器，基于 Dired 模式，提供增强的界面和功能，如多窗格布局、预览、图标支持等。
*** 相关链接
...
** ultra-scroll
*** 介绍
ultra-scroll 提供平滑滚动体验，避免跳跃式滚动。它通过微调滚动行为，使内容移动更自然。
*** 相关链接
...
** markdown-mode
*** 介绍
markdown-mode 是 Emacs 中用于编辑 Markdown 文件的主流模式，支持 GitHub Flavored Markdown (GFM) 语法扩展。它提供语法高亮、预览、快捷键等功能，提升 Markdown 编辑体验。
*** 相关链接
...
#+end_src
*** 使用方法
- 自动启用 markdown-mode 编辑 .md 和 .markdown 文件
- 使用 =M-x markdown-preview-mode= 预览 Markdown 渲染效果
...
** beacon
*** 介绍
beacon 在光标移动时高亮显示光标位置，方便快速定位。特别适合大屏幕或多窗口环境。
*** 相关链接
...
** evil-matchit
*** 介绍
evil-matchit 允许在 Evil 模式下快速跳转到匹配的标签或括号，类似 Vim 的 matchit 插件。支持多种编程语言和标记语言。
*** 相关链接
...
*** 介绍
evil-anzu 集成了 anzu.el 的搜索增强功能到 Evil 模式中，提供更直观的搜索反馈。显示当前搜索匹配项的位置和总数，支持增量搜索。
*** 相关链接
...
** consult
*** 介绍
consult 是一个强大的补全和导航工具，提供文件查找、buffer 切换、命令执行等功能。与 vertico、orderless、marginalia 完美集成。
*** 相关链接
...
*** 介绍
org-fancy-priorities 用于在 org-mode 中使用自定义符号显示任务优先级，提升视觉效果和可读性。
*** 相关链接
...
#+end_src
*** 使用方法
- 在 org-mode 中使用 =C-c , p= 设置任务优先级


** auctex

*** 介绍
AUCTeX 是 Emacs 中功能强大的 LaTeX 编辑环境，提供语法高亮、自动补全、预览、编译等功能。支持多种 LaTeX 发行版和工具链。
*** 相关链接
- 官网: https://www.gnu.org/software/auctex/
- 文档: https://www.gnu.org/software/auctex/manual/
*** 配置
#+begin_src emacs-lisp
(use-package auctex
  :defer t
...
- =C-c C-v= : 预览生成的 PDF
- =C-c= : 插入数学符号
- =M-n/M-p= : 在 LaTeX 环境间导航

** math-preview

*** 介绍
math-preview 是一个 Emacs 插件，用于实时预览 LaTeX 数学公式。支持行内公式和块级公式，生成 SVG 图像并嵌入缓冲区。
*** 相关链接
...
#+end_src
*** 使用方法
- 在 org-mode 中输入 LaTeX 数学公式（行内用 $...$，块级用 $$...$$ 或 \[...\]）
- 光标离开公式后，公式会自动渲染为 SVG 图像
- 光标进入公式时，预览会被清除，方便编辑
...
** org-sliced-images
*** 介绍
org-sliced-images 为 org-mode 提供平滑的图片滚动体验，通过将图片切片显示来实现流畅滚动。相比 iscroll 更快且更原生，不需要重新绑定移动键。
*** 相关链接
- GitHub: https://github.com/jcfk/org-sliced-images
- 对比: 比 iscroll 更快，比 pixel-scroll 更省资源
*** 配置
#+begin_src emacs-lisp
...
#+end_src
*** 使用方法
- 启用后自动对 org-mode 中的内联图片进行切片处理
- 使用正常的移动命令 (=j/k=, =C-n/C-p=, 鼠标滚轮) 即可平滑滚动图片
- 切换图片显示: =C-c C-x C-v= (org-toggle-inline-images)
- 建议配合 =org-startup-with-inline-images= 自动显示图片
*** 注意事项
- 如果使用 org-indent-mode 或行号，必须设置 =org-sliced-images-round-image-height= 为 =t=
- 只影响 org-mode 的内联图片，不影响其他模式
...
*** 介绍
apheleia 是一个 Emacs 代码格式化工具，支持多种编程语言和格式化器。它可以在保存文件时自动格式化代码，提升代码一致性
*** 相关链接
...
*** 使用方法
- 保存文件时自动格式化
- =M-x apheleia-format-buffer= : 手动格式化当前缓冲区
...
** eldoc-box
*** 介绍
eldoc-box 为 eldoc 提供弹出框显示，支持多行内容和滚动。相比内置 eldoc，更加美观和实用。
*** 相关链接
...
*** 使用方法
- =SPC c h= : 显示 eldoc 弹出框
- =SPC c j/k= : 向上/下滚动 eldoc 内容

** flymake
*** 介绍
flymake 是 Emacs 内置的语法检查框架，支持异步检查代码错误。与 lsp-mode 集成后，可以显示 LSP 提供的诊断信息。
*** 相关链接
- 官方文档: https://www.gnu.org/software/emacs/manual/html_node/flymake
*** 配置
#+begin_src emacs-lisp
  (use-package flymake
  :straight nil
...
- 自动在编程模式启用 flymake
- 显示语法错误和警告
- 与 lsp-mode 集成显示 LSP 诊断信息


** flyover
*** 介绍
flyover 在代码行旁边显示诊断信息的虚拟行，支持 flycheck 和 flymake。提供丰富的外观和行为定制选项。
*** 相关链接
...
#+end_src
*** 使用方法
- 自动在启用 flycheck/flymake 时显示虚拟行
- 显示错误、警告和信息图标

** lsp-kdl

*** 介绍
lsp-kdl 为 KDL（Kotlin Data Language）提供 LSP 支持，基于 tree-sitter 解析器，支持语法高亮、代码补全等功能。
...
** quickrun
*** 介绍
quickrun 提供快速运行代码片段的功能，支持多种编程语言。可以在 Emacs 中直接执行代码并查看输出，适合测试和调试小段代码。
...
*** 使用方法
- =M-x quickrun= : 运行当前缓冲区代码
- =M-x quickrun-region= : 运行选中代码
- 支持多种语言，自动选择运行器
...
** Pomidor 番茄钟
*** 介绍
Pomidor 是一个 Emacs 番茄钟插件，帮助用户专注工作和管理时间。它提供可视化的计时器和休息提醒，支持自定义工作和休息时间。
*** 相关链接
...
*** DONE math-preivew 的bug,当 font-height 设置大160时，预览会偏移
:LOGBOOK:
