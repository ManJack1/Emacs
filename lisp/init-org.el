...
(use-package
 ...
 ((org-mode . visual-line-mode)
  (org-mode . yas-minor-mode) (org-mode . laas-mode)
 ...
 (setq
  ...
  '(:foreground
    ...
    :html-scale 1.0
    :matchers ("begin" "$1" "$" "$$" "\\(" "\\["))
  org-latex-preview-appearance-options '(:page-width 0.8 :zoom 1.0 :org-latex-preview-numbered t))

 ;; LaTeX导出设置
 (setq
  org-latex-listings t
  org-latex-src-block-backend 'minted
  org-latex-pdf-process '("latexmk -xelatex -f -interaction=nonstopmode %f")
...
(defun org-justify-fragment-overlay
  ...
  (when (and (= beg (line-beginning-position))
    ...
    (let* ((ov (ov-at))
      ...
      (overlay-put
       ov 'line-prefix `(space :align-to (- center (0.5 . ,disp)))))))
        ...
        (progn
          ...
          (when (file-exists-p local-path)
            ...
            (when (fboundp 'org-display-inline-images)
              (org-display-inline-images))))
      (message
       "Screenshot not supported on this platform or missing tools"))))
...
(defun my/paste-image-from-clipboard ()
  ...
  (if (fboundp 'org-download-clipboard)
      (org-download-clipboard)
    (message
     "Clipboard image pasting requires org-download. Use org-download-clipboard manually.")))
 ...
 (setq cdlatex-command-alist
       '(
         ...
         ("lr|"
          ...
          nil)
         ("lra"
          "Insert left-right angle brackets"
...
(setq org-preview-latex-default-process
      (cond
       ((eq system-type 'gnu/linux)
        'dvisvgm) ; Linux 使用 dvisvgm
       ((eq system-type 'darwin)
        'imagemagick) ; macOS 使用 imagemagick
       (t
        'dvipng)))
...
(when (eq system-type 'darwin)
  ...
  (advice-add
   ...
   :filter-return #'my/image-scale-advice)
  ...
  (defun my/overlay-scale-advice (beg end image &optional imagetype)
    ...
    (mapc
     (lambda (ov)
       (when (equal
              (overlay-get ov 'org-overlay-type) 'org-latex-overlay)
         (overlay-put
          ov 'display
          (list
           'image
...
(eval-after-load "preview"
  '(add-to-list
    'preview-default-preamble "\\PreviewEnvironment{tikzpicture}"
    t))
 ...
 (setq org-todo-keywords
       (quote ((sequence "TODO(t)" "STARTED(s)" "|" "DONE(d!/!)")
               (sequence
                ...
                "MEETING(m)"
                "PHONE(p)"))))
 (setq org-log-done t) (setq org-log-into-drawer t))
...
(use-package
 ...
 (add-hook
  'org-mode-hook
  (lambda () (run-with-idle-timer 0.1 nil 'toggle-word-wrap))))
