...
(with-eval-after-load 'evil
  (my/leader-keys
   ...
   "cs"
   'db/lsp-treemacs-symbols-toggle ; 添加 LSP treemacs symbols
...
(with-eval-after-load 'evil
  ...
  ;; 错误导航
  (evil-define-key
   'normal 'global (kbd "[d") 'flymake-goto-prev-error)
  (evil-define-key
   'normal 'global (kbd "]d") 'flymake-goto-next-error)

  ;; 快速注释
  (evil-define-key
   '(normal visual) 'global (kbd "gc") 'comment-or-uncomment-lines)
  (evil-define-key
   'visual 'global (kbd "gc") 'comment-or-uncomment-region))
;; 方案一：完全解绑 M-c（推荐）
...
(use-package
 ...
 (define-key
  dirvish-mode-map (kbd "M-b") 'dirvish-history-go-backward)
 ...
 (with-eval-after-load 'evil
   (evil-define-key
    ...
    'dirvish-dispatch
    "A"
    'dirvish-setup-menu ; Keep original A for dirvish setup
    "f"
    'dirvish-file-info-menu
    "s"
    'dirvish-quicksort
    "R"
    'dirvish-history-jump ; Move history to R
    "v"
    ...
    "a"
    'my-dirvish-create-file ; add file/directory
    "d"
    'my-dirvish-delete ; delete
    "r"
    'my-dirvish-rename ; rename
    "y"
    'my-dirvish-copy ; yank/copy
    "x"
    'my-dirvish-cut ; cut
    "p"
    'my-dirvish-paste ; paste
...
If path ends with '/', create a directory; otherwise create a file."
  ...
  (let* ((current-dir (dired-current-directory))
         (input
          (read-string "Create (end with / for directory): "
                       current-dir))
  ...
  (let* ((current-dir (dired-current-directory))
    ...
    (when (and dirname (not (string-empty-p dirname)))
      ...
      (let ((path (expand-file-name dirname)))
        (if (file-exists-p path)
            (message "Directory already exists: %s"
                     (file-name-nondirectory
                      (directory-file-name path)))
          (make-directory path t)
          (revert-buffer)
...
(defun my-find-config-files ()
  ...
  (let* ((config-dir user-emacs-directory)
         ...
         (files
          (append
           ...
           (cl-remove-if-not
            #'file-exists-p
            (mapcar
             (lambda (f) (expand-file-name f config-dir))
    ...
    (if choices
        (let* ((selected
                (completing-read "Open config file: " choices))
